	...
	if (IsCTFFilter == 1) {
		$.ajax({
			...
			success: function (response, opts) {
				...
				$.ajax({
					url: coolerDashboard.common.nodeUrl('getCustomerTierWidgetData'),
					data: filterValuesChart,
					type: 'POST',
...
$(function () {
	...
	var mapDiv = document.getElementById('map-canvas-customer');
	...
	var sparklineRenderer2 = function (value, data, row) {
		return '<span class="sparkline-type-pie cdeTracking" data-sparkline-piesize="23px">' +
	...
	var otable2 = $('#outletPerformance').dataTable({
		ajax: {
			url: './js/ajax/coolerStatus.txt',
			dataSrc: 'data.outletPerformance',
			serverSide: false
		},
		"bPaginate": true,
		"bFilter": false,
		...
		"sDom": "" +
			"t" +
			"<'dt-toolbar-footer'<'col-sm-12 col-lg-12 col-xs-12 well-sm'i><'col-sm-8 col-xs-8 hidden-xs'l><'col-xs-12 col-sm-4'p>>",
		"oLanguage": {
			"sSearch": '<span class="input-group-addon"><i class="glyphicon glyphicon-search"></i></span>'
		},
		"preDrawCallback": function () {
	...
	var otable2 = $('#cdeTracking').dataTable({
		...
		"drawCallback": function (oSettings) {
			...
			$('.cdeTracking').sparkline('html', {
				type: 'pie',
				disableHiddenCheck: true,
				height: '18px',
				width: '18px'
			}).removeClass('sparkline-type-pie cdeTracking');;
