...
module.exports = {
	...
	dashboardQueries: {
		assetSummary: JSON.stringify(require('./dashboardQueries/customerTier/salesAssetSummary.json')),
		locationSummary: JSON.stringify(require('./dashboardQueries/customerTier/kpilocationSummary.json')),
		classificationSummary: JSON.stringify(require('./dashboardQueries/customerTier/kpiclassificationSummary.json')),
		locationTypeSummary: JSON.stringify(require('./dashboardQueries/customerTier/kpilocationtypeSummary.json')),
		momentSummary: JSON.stringify(require('./dashboardQueries/customerTier/kpiMovementSummaryCustomerTier.json')),
		visitSummary: JSON.stringify(require('./dashboardQueries/customerTier/kpiVisitSummaryCustomerTier.json')),
		powerSummary: JSON.stringify(require('./dashboardQueries/customerTier/customerPowerSummary.json')),
		assetVisitSummary: JSON.stringify(require('./dashboardQueries/customerTier/assetVisitSummaryCustomerTier.json')),
		doorSummary: JSON.stringify(require('./dashboardQueries/customerTier/customerDoorSummary.json')),
		alertSummary: JSON.stringify(require('./dashboardQueries/customerTier/customerTierAlertSummary.json')),
		kpiSalesSummary: JSON.stringify(require('./dashboardQueries/customerTier/kpiSalesSummaryCustomerTier.json')),
		kpiSalesSummaryTrend: JSON.stringify(require('./dashboardQueries/customerTier/kpiSalesSummaryTrendCustomerTier.json')),
		TotalAssetLocation: JSON.stringify(require('./dashboardQueries/TotalAssetLocation.json'))
	},

	getCustomerTierWidgetData: function (request, reply) {
		...
		var params = Object.assign({}, request.query, request.payload),
			...
			classificationSummary = JSON.parse(this.dashboardQueries.classificationSummary),
		...
		if (clientId != 0) {
			...
			classificationSummary.query.bool.filter.push(clientQuery);
		}
		//EventDate Filter		
		...
		if ((params.telemetryDoorCount || params["telemetryDoorCount[]"]) ||
			...
			locationSummary.query.bool.filter.push(AssetIds);
			...
			visitSummary.query.bool.filter.push(AssetIds);
			alertSummary.query.bool.must.push(AssetIds);
			kpiSalesSummary.query.bool.filter.push(AssetIds);
			kpiSalesSummaryTrend.query.bool.filter.push(AssetIds);
		}
		...
		if ((params.DoorSwingsVsTarget || params["DoorSwingsVsTarget[]"]) ||
			...
			locationSummary.query.bool.filter.push(LocationIds);
			...
			visitSummary.query.bool.filter.push(LocationIds);
			alertSummary.query.bool.must.push(LocationIds);
			kpiSalesSummary.query.bool.filter.push(LocationIds);
			kpiSalesSummaryTrend.query.bool.filter.push(LocationIds);
		}
		...
		if (request.query.SmartDeviceManufacturerId || request.query["SmartDeviceManufacturerId[]"]) {
			...
			assetVisitSummary.query.bool.filter.push(manufacturerSmartDeviceQuery);
			...
			visitSummary.query.bool. filter.push(manufacturerSmartDeviceQuery); //for visit compliance
		}
		...
		Promise.all(promises).then(function (values) {
			...
			var alertAggs = data.alert.aggregations,
				...
				classification = data.classification.aggregations.ClassificationBucket.buckets;
			...
			classification.forEach(function (classificationBucket) {
				classificationData.push(classificationBucket.ClassificationId.hits.hits[0]._source);
			});
			...
			var classificationLocation = dbAggs.AssetCountTotal.ByCityoutlet.buckets;
			var classificationAsset = dbAggs.AssetCountTotal.ByCity.buckets;
			...
			if (assetVisitAggs) {
				assetVisitAggs.Classification.buckets.forEach(function (bucket) {
					cdeTrackingMatched = finalData.cdeTracking.filter(data => data.ClassificationId == bucket.key);
					var classification = classificationData.filter(data => data.ClassificationId == bucket.key);
					if (classification.length > 0) {
						classification = classification[0].Classification
					} else {
						classification = 'No Tier';
						cdeTrackingMatched = finalData.cdeTracking.filter(data => data.CustomerTier == classification);
					}
					//if (locationDataMissing[0]) {
					if (cdeTrackingMatched.length == 0) {
						finalData.cdeTracking.push({
							"CustomerTier": classification,
							"ClassificationId": bucket.key,
							"MissingCooler": bucket.key,
							"CoolerMovement": '',
						...
						if (index != -1) {
							finalData.cdeTracking[index].MissingCooler += bucket.key;
						}
					}
					//}
				});
			...
			if (movementAggs) {
				movementAggs.Classification.buckets.forEach(function (bucket) {
					cdeTrackingMatched = finalData.cdeTracking.filter(data => data.ClassificationId == bucket.key);
					var classification = classificationData.filter(data => data.ClassificationId == bucket.key);
					if (classification.length > 0) {
						classification = classification[0].Classification
					...
					if (cdeTrackingMatched.length == 0) {
						finalData.cdeTracking.push({
							"CustomerTier": classification,
							"MissingCooler": '',
							"ClassificationId": bucket.AssetCount.value,
			...
			for (var i = 0; i < classificationData.length; i++) {
				var vrTier = classificationData[i].Classification;

				//if (vrTier.length > 0) {
				var vrIndexID = finalData.cdeTracking.findIndex(x => x.CustomerTier == vrTier)
			...
			if (visitAggs) {
				visitAggs.Classification.buckets.forEach(function (visitData) {
					...
					routeComplianceMatched = finalData.routeCompliance.filter(data => data.ClassificationId == visitData.key);
					var classification = classificationData.filter(data => data.ClassificationId == visitData.key);
					if (classification.length > 0) {
						classification = classification[0].Classification
					} else {
						classification = 'No Tier';
						routeComplianceMatched = finalData.routeCompliance.filter(data => data.CustomerTier == classification);
					}
					if (routeComplianceMatched.length == 0) {
						finalData.routeCompliance.push({
							"CustomerTier": classification,
							"ClassificationId": visitData.key,
							"VisitDuration": sumDuration,
							"VisitPerMonth": visitData.SumDuration.value,
			...
			for (var i = 0; i < classificationData.length; i++) {
				...
				//if (vrTier.length > 0) {
				var vrIndexID = finalData.routeCompliance.findIndex(x => x.CustomerTier == vrTier)
			...
			if (alertAggs) {
				alertAggs.Classification.Classification.buckets.forEach(function (bucket) {
					alertDataMatched = finalData.coolerStatus.filter(data => data.ClassificationId == bucket.key);
					var classification = classificationData.filter(data => data.ClassificationId == bucket.key);
					if (classification.length > 0) {
						classification = classification[0].Classification
					} else {
						classification = 'No Tier';
						alertDataMatched = finalData.coolerStatus.filter(data => data.CustomerTier == classification);
					}
			...
			var locationDataHealth, locationDataMatched,
				totalAssets = dbAggs.AssetCount.AssetCount.buckets.length;
			...
			if (powerAgggs) {
				powerAgggs.Classification.buckets.forEach(function (locationBucket) {
					...
					locationDataMatched = finalData.coolerStatus.filter(data => data.ClassificationId == locationBucket.key);
					var classification = classificationData.filter(data => data.ClassificationId == locationBucket.key);
					if (classification.length > 0) {
						classification = classification[0].Classification
					} else {
						classification = 'No Tier';
						locationDataMatched = finalData.coolerStatus.filter(data => data.CustomerTier == classification);
					}
					if (locationDataMatched.length == 0) {
						finalData.coolerStatus.push({
							"CustomerTier": classification,
							"ClassificationId": locationBucket.key,
							"HourlyDoorOpen": '',
							"DoorOpenDuration": '',
							"TempHours": '',
			...
			var customerTierData = [];

			if (doorAggs) {
				doorAggs.Classification.buckets.forEach(function (locationBucket) {
					locationDataMatched = finalData.coolerStatus.filter(data => data.ClassificationId == locationBucket.key);
					var classification = classificationData.filter(data => data.ClassificationId == locationBucket.key);
					if (classification.length > 0) {
						classification = classification[0].Classification
			...
			finalData.coolerStatus.forEach(function (termBucket) {
				...
				termBucket.HourlyDoorOpen = termBucket.HourlyDoorOpen / termBucket.DoorAssetCount / totalHours;
			...
			for (var i = 0; i < classificationData.length; i++) {
				var vrCEDUtilization = classificationData[i].Classification;
				//if (vrCEDUtilization.length > 0) {
				var vrIndexID = finalData.coolerStatus.findIndex(x => x.CustomerTier == vrCEDUtilization)

				if (vrIndexID == -1) {
					finalData.coolerStatus.push({
						"CustomerTier": vrCEDUtilization,
						"HourlyDoorOpen": '',
						"DoorOpenDuration": '',
						"TempHours": '',
						"PowerOn": '',
			...
			if (salesAggs) {
				salesAggs.Classification.buckets.forEach(function (locationBucket) {
					locationDataMatched = finalData.outletPerformance.filter(data => data.ClassificationId == locationBucket.key);
					var classification = classificationData.filter(data => data.ClassificationId == locationBucket.key);
					if (classification.length > 0) {
						classification = classification[0].Classification
					} else {
						classification = 'No Tier';
						locationDataMatched = finalData.outletPerformance.filter(data => data.CustomerTier == classification);
					}
					if (locationDataMatched.length == 0) {
			...
			if (salesTrendAggs) {
				salesTrendAggs.Classification.buckets.forEach(function (locationBucket) {
					locationDataMatched = finalData.outletPerformance.filter(data => data.ClassificationId == locationBucket.key);
					var classification = classificationData.filter(data => data.ClassificationId == locationBucket.key);
					if (classification.length > 0) {
						classification = classification[0].Classification
			...
			for (var i = 0; i < classificationData.length; i++) {
				...
				//if (vrTier.length > 0) {
				var vrIndexID = finalData.outletPerformance.findIndex(x => x.CustomerTier == vrTier)
			...
			finalData.coolerStatus.forEach(function (tier) {
				...
				activityGridData = finalData.outletPerformance.filter(data => data.CustomerTier == tier.CustomerTier);
			...
			//Start : Resolve bug ID 5073 : Joyal : 05-Dec-2016
			for (var i = 0; i < classificationData.length; i++) {
				var vrActivity = classificationData[i].Classification;


				//if (vrActivity.length > 0) {
				var vrIndexID = finalData.activityGrid.findIndex(x => x.CustomerTier == vrActivity)

				if (vrIndexID == -1) {
					finalData.activityGrid.push({
						"CustomerTier": vrActivity,
						"DoorOpenHourly": '',
						"DoorCount": '',
						"SalesVolume": ''
					});
			...
			var smartAssetCount = data.db.aggregations.SmartAssetCount.SmartAssetCount.buckets.length;
			finalData.summary = {
