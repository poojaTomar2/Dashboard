...
module.exports = {
	...
	dashboardQueries: {
		...
		salesCorelationPowerSummary: JSON.stringify(require('./dashboardQueries/salesCorelationPowerSummary.json')),
	...
	getSalesCorrelationWidgetData: function (request, reply) {
		...
		var params = Object.assign({}, request.query, request.payload),
			...
			salesCorelationPowerSummary = JSON.parse(this.dashboardQueries.salesCorelationPowerSummary),
		...
		if (clientId != 0) {
			...
			salesCorelationPowerSummary.query.bool.filter.push(clientQuery);
		...
		if (!isDefaultDateFilter && !params.startDate && !params.endDate) {
			...
			salesCorelationPowerSummary.query.bool.filter.push({
				"range": {
					"EventTime": {
						"gte": "now-30d/d"
					}
		...
		for (var i = 0, len = dateFilter.length; i < len; i++) {
			...
			salesCorelationPowerSummary = util.pushDateQuery(salesCorelationPowerSummary, {
				"range": {
					"EventTime": {
						"gte": startDate,
						"lte": endDate
		...
		util.applyReducers(request, params, totalHours, reducers, function (_this, assetIds, locationIds) {
			if (Array.isArray(locationIds)) {
				...
				salesCorelationPowerSummary.query.bool.filter.push(locationTerm);
			...
			if (limitLocation != 0) {
				...
				salesCorelationPowerSummary.query.bool.filter.push(filterQuery);
			...
			if (request.query.AssetTypeId || request.query["AssetTypeId[]"]) {
				...
				salesCorelationPowerSummary.query.bool.filter.push(smartDeviceTypeQuery);
			}
			...
			if (request.query.AssetManufacturerId || request.query["AssetManufacturerId[]"]) {
				...
				salesCorelationPowerSummary.query.bool.filter.push(assetManufactureQuery);
			}
			...
			if (request.query.SmartDeviceManufacturerId || request.query["SmartDeviceManufacturerId[]"]) {
				...
				salesCorelationPowerSummary.query.bool.filter.push(manufacturerSmartDeviceQuery);
			}
			...
			if (assetIds) {
				...
				salesCorelationPowerSummary.query.bool.filter.push(assetQuery);
			}
			...
			var queries = [{
				...
				search: {
					...
					body: salesCorelationPowerSummary,
			...
			Promise.all(promises).then(function (values) {
				...
				var dbAggs = data.db.aggregations,
					powerAggs = data.power.aggregations,
				...
				var powerDay;
				var powerAsset = 0;
				var monthDays, doorData, doorOpenTarget, salesTarget;

				if (powerAggs) {
					powerAggs.Power.buckets.forEach(function (dayBucket) {
						powerOpenHour = 0;
						salesData = data.sales.aggregations.Sales.buckets.filter(data => data.key == dayBucket.key);
						doorData = undefined;
						if (doorData) {
							doorData = data.door.aggregations.Door.buckets.filter(data => data.key == dayBucket.key);
						}
						doorOpenTarget = 0;
						salesTarget = 0;
						...
						doorOpenTarget = doorOpenTarget * moment(dayBucket.key).daysInMonth();
						powerAsset = dayBucket.AssetCount.value;
						powerOffHour = moment.duration(dayBucket.PowerOffDuration.value, 'second').asHours();
						monthDays = moment(dayBucket.key).daysInMonth();
						powerOpenHour = (24 * monthDays * powerAsset - (powerOffHour > (24 * monthDays) ? (24 * monthDays) : powerOffHour < 0 ? 0 : powerOffHour)) / powerAsset;

						finalData.powerOnSales.push({
							"Date": dayBucket.key,
							"DateValue": dayBucket.key_as_string,
				...
				if (salesAggs) {
					salesAggs.Sales.buckets.forEach(function (sales) {
						salesData = finalData.powerOnSales.find(data => data.Date == sales.key);
						doorData = undefined;
						if (doorAggs) {
				...
				if (doorAggs) {
					doorAggs.Door.buckets.forEach(function (door) {
						...
						salesTarget = 0;
						doorData = undefined;
						salesData = finalData.powerOnSales.find(data => data.DateValue == door.key_as_string);
